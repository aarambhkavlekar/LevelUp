...
// NOTE: Assuming 'login_page_emp.dart' is the correct login page for employees/recruiters
...
class _SignInPageState extends State<SignInPage> {
  ...
  Future<void> _submitData() async {
    ...
    try {
      // Add data to a 'recruiters' collection
      await FirebaseFirestore.instance.collection('recruiters').add({
        'name': _nameController.text.trim(),
        'cin_no': _cinController.text.trim(),
        'contact': _contactController.text.trim(),
        'email': _emailController.text.trim(),
        'password': _passwordController.text.trim(), // In a real app, hash this!
        'createdAt': Timestamp.now(),
      });
      final cinno = _cinController.text.trim();
      final db = FirebaseFirestore.instance;
      final querySnapshot = await db.collection('recruiters') // <-- Targeting 'recruiters' collection
      ...
      if (mounted) {
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (context) => RecruiterDashboard(userData)),
        );
      }
    } catch (e) {
      if (mounted) {
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // Name Input
                          _buildInputField(controller: _nameController, labelText: "Name"),
